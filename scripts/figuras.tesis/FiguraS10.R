...
if (dir.exists(paste0(wkdir, "/Figuras_tesis/FiguraS10")) == F) {

    dir.create(paste0(wkdir, "/Figuras_tesis/FiguraS10"))

}


outdir <- paste0(wkdir, "/Figuras_tesis/FiguraS10")
...
df <- data.frame("Modelo.Cox" = unlist(cindexm1), "Modelo.Cox.SNs" = unlist(cindexm2))

df <- df %>% pivot_longer(c(Modelo.Cox, Modelo.Cox.SNs),names_to = "Model", values_to = "C.index")
...
plt <- plt + scale_fill_manual(values = list("Modelo.Cox" = "darkslategray3",
    "Modelo.Cox.SNs" = "slateblue3")) +
