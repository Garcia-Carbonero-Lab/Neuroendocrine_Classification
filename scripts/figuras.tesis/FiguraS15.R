...
if (dir.exists(paste0(wkdir, "/Figuras_tesis/FiguraS15")) == F) {

    dir.create(paste0(wkdir, "/Figuras_tesis/FiguraS15"))

}


outdir <- paste0(wkdir, "/Figuras_tesis/FiguraS15")
...
deg3_filter <- rownames(deg3)[deg3$adj.P.Val < 0.1 & abs(deg3$logFC) > 0.2]
deg2_filter <- rownames(deg2)[deg2$adj.P.Val < 0.05 & abs(deg2$logFC) > 0.5]
deg1_filter <- rownames(deg1)[deg1$adj.P.Val < 0.05 & abs(deg1$logFC) > 0.5]
